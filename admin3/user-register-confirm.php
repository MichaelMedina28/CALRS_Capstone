...
$sql2 = "SELECT user_id FROM user_tbl ORDER BY user_id DESC LIMIT 1";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $user_pass = password_hash($_POST['user-pass'], PASSWORD_DEFAULT);
    ...
    //Proof of Share on Investment
    $file2 = $_FILES["user-proof-soi"];
    // File details
    $file2Name = "$user_firstName $user_middleName $user_lastName - (PROOF_SOI)" .$file2["name"];
    $file2TmpName = $file2["tmp_name"];
    $file2Size = $file2["size"];
    $file2Error = $file2["error"];

    $file2destination = "../admin/proof_soi/" . $file2Name;
    ...
    $share_investment = number_format($share_investment, 2);
    ...
    account_pictures,
    user_mnumber, user_email, user_pdocument_type, pdocument, position, spouse_name, share_investment, proof_soi) 
    VALUES ('$user_id','$user_pass','$user_pass_confirm', '$user_firstName', 
    '$user_middleName', '$user_lastName', '$user_suffixName',
    '$user_birthdate', '$user_address_room', '$user_address_house', '$user_address_street', '$user_address_subd', '$user_address_brgy',
    '$destination',
    '$user_mnumber', '$user_email', '$user_pdocument_type', '$pdocument', '$user_position', '$user_spouseName', '$share_investment' , '$file2destination')";
                ...
                <div class="container-fluid">
                    ...
                    <div class="card shadow">
                            <div class="card-header py-3">
                                <p class="text-primary m-0 fw-bold"><i class="fas fa-check-circle me-2"></i>Register Confirm</p>
                            </div>
                            <div class="card-body">
                                <p class="p-text fw-bold">Successfully Registered User</p>
                            </div>
                            <div class="card-footer d-xl-flex justify-content-xl-end"><a class="btn btn-secondary btn-icon-split m-1" type="submit" href="user-approved.php"><span class="text-white-50 icon"><i class="fas fa-chevron-circle-left"></i></span><span class="text-white text">Back</span></a></div>
                        </div>
                    </form>
                </div>
            </div>
