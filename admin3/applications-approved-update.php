...
if (isset($_POST['update'])) {
    ...
    if ($stmt = mysqli_prepare($conn, $sql)) {
        ...
        $description1 = "The disbursement status of loan " . $loanId . " is now For Release";
        $logSql = "INSERT INTO login_logs (user_id, notif_description) VALUES ('$adminUsername', '$description1')";
        $conn->query($logSql); // Assuming $conn is your database connection

        //For notif Logs
        $description = "The disbursement status of your loan " . $loanId . " is now For Release";
        $logSql1 = "INSERT INTO notif_logs (user_id, notif_description, approver) VALUES ('$user_id', '$description', '$adminUsername')";
...
if (isset($_POST['release'])) {
    ...
    //For Activity Logs
    $adminUsername = $_SESSION["admin_username"];
    $description = "The disbursement status of $loanId is now Released";
    $logSql = "INSERT INTO login_logs (user_id, notif_description) VALUES ('$adminUsername', '$description')";
    $conn->query($logSql); // Assuming $conn is your database connection

    //For Login Logs
    $description2 = "You can now check your current active loan.";
    $logSql2 = "INSERT INTO notif_logs (user_id, notif_description, approver) VALUES ('$user_id', '$description2', '$adminUsername')";
    $conn->query($logSql2); // Assuming $conn is your database connection

    //For Login Logs
    $description = "The disbursement of your loan ($loanId) has been Released. Please proceed to the office of CRG-MPC.";
    $logSql1 = "INSERT INTO notif_logs (user_id, notif_description, approver) VALUES ('$user_id', '$description', '$adminUsername')";
    $conn->query($logSql1); // Assuming $conn is your database connection
                    ...
                    <form id="myForm" method="post">
                        <div class="card shadow">
                            <div class="card-header py-3">
                                <p class="text-primary m-0 fw-bold"><i class="fas fa-info-circle me-2"></i>Approved</p>
                            </div>
                            <div class="card-body">
                                ...
                                <div class="row">
                                    <div class="col d-xl-flex align-items-xl-center col-md-6">
                                        <div class="form-check">
                                            ...
                                            <select class="form-select d-lg-flex" id="ca-status" name="ca-status">
                                                <?php
                            ...
                            <div class="card-footer">
                                ...
                                <a class="btn btn-primary btn-icon-split m-1" id="print-ds" href="print-ds.php?id=<?php echo $row["loan_id"];?>.php"><span class="text-white-50 icon"><i class="fas fa-print"></i></span><span class="text-white text">Print DS</span></a>

                                <button class="btn btn-info btn-icon-split m-1" name="update" id="update-btn" type="submit"><span class="text-white-50 icon"><i class="fas fa-arrow-alt-circle-up text-light"></i></span><span class="text-white text">Update</span></button>
                                <button class="btn btn-success btn-icon-split m-1" name="release" id="approve-btn" type="submit" disabled><span class="text-white-50 icon"><i class="fas fa-check-circle"></i></span><span class="text-white text">Release</span></button>
                            </div>
...
//buttons to hide 
    ...
    var releaseButton = document.getElementById("approve-btn");
