...
if (isset($_POST['disapprove-btn'])) {
    ...
    $description = 'Disapproved your Application Form (' . $loanId . ')';

    $sql = "INSERT INTO app_disapproved_tbl SELECT * FROM lo_app_pending_tbl where loan_id = ?";
    $sql2 = "DELETE from lo_app_pending_tbl where loan_id = ?";
    ...
    // Execute statement 1 and check for errors
    if (mysqli_stmt_execute($stmt)) {
        // Execute statement 2 and check for errors
        if (mysqli_stmt_execute($stmt2)) {
            ...
            if ($conn->query($logSql) === TRUE) {
                ...
                if ($conn->query($logSql1) === TRUE) {
                    ...
                    // Redirect to the applications-pending.php page
                    header("Location: applications-pending.php");
                    exit;
                } else {
                    echo "Error inserting into notif_logs table: " . $conn->error;
                }
            } else {
                echo "Error inserting into login_logs table: " . $conn->error;
            }
        } else {
            echo "Error executing statement 2: " . mysqli_stmt_error($stmt2);
        }
    } else {
        echo "Error executing statement 1: " . mysqli_stmt_error($stmt);
    }
}

$renewal_balance_amount = "";
if (isset($_POST['approve-btn'])) {
    $loanId = $_GET['id'];
    $seq_nos = $_POST['seq-nos'];
    $lot_nos = $_POST['lot-nos'];
    $renewal_balance_amount = $_POST['renewal-balance-amount'];
    
    $adminUsername = $_SESSION["admin_username"];
    $description = 'Approved your Application Form (' . $loanId . '). Please wait for final approval.';

    $sqlnotif = "INSERT INTO notif_logs (user_id, notif_description, approver) VALUES ('$user_id', '$description', '$adminUsername')";
    $conn->query($sqlnotif);

    $tdeduction = floatval($da_deduction_amount) + floatval($da_sfee_amount) + floatval($da_cbu_amount) + floatval($da_insurance_amount) + floatval($renewal_balance_amount);
    $tdeductionfloat = floatval($tdeduction);
    ...
    $differenceraw = $amount_number - $tdeductionfloat;
    $difference = number_format($differenceraw, 2);

    if($pn_rate_payment === "12 Months"){
        $halfnetProceed = $differenceraw;
        $halfnetProceed = number_format($halfnetProceed, 2);
        $amortamount = $halfnetProceed;
    } elseif($pn_rate_payment === "6 Months"){
        $halfnetProceed = $differenceraw / 2;
        $halfnetProceed = number_format($halfnetProceed, 2);
        $amortamount = $halfnetProceed;
    } elseif ($pn_rate_payment === "4 Months"){
        $monthlynetProceed = $differenceraw / 4;
        $monthlynetProceed = number_format($monthlynetProceed, 2);
        $amortamount = $monthlynetProceed;
    }elseif ($pn_rate_payment === "1 Month"){
        $monthlynetProceed = $differenceraw / 12;
        $monthlynetProceed = number_format($monthlynetProceed, 2);
        $amortamount = $monthlynetProceed;
    }
    

    $updatesql = "UPDATE lo_app_pending_tbl SET seq_nos = ?, lot_nos = ?, da_other_amount = ?, da_tdeduction_amount = ? , da_nproceed_amount = ?, da_amortization_amount1 = ?, al_amount_balance = ? WHERE loan_id = ?";
    ...
    $sql = "INSERT INTO app_pending_tbl SELECT * FROM lo_app_pending_tbl where loan_id = ?";
    $sql2 = "DELETE from lo_app_pending_tbl where loan_id = ?";
  ...
  // Bind the parameters
  mysqli_stmt_bind_param($stmt3, "sssssssi", $seq_nos, $lot_nos, $renewal_balance_amount, $tdeduction, $difference, $amortamount , $difference, $loanId);
  mysqli_stmt_bind_param($stmt, "i", $loanId);
  mysqli_stmt_bind_param($stmt2, "i", $loanId);
                    ...
                    <form method="post">
                        <div class="card shadow">
                            <div class="card-header py-3">
                                <p class="text-primary m-0 fw-bold"><i class="fas fa-info-circle me-2"></i>New Submissions</p>
                            </div>
                            <div class="card-body">
                                <div class="col">
                                    ...
                                    <div class="modal fade" role="dialog" tabindex="-1" id="disapprove-modal">
                                        <div class="modal-dialog modal-dialog-centered" role="document">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                    <p class="p-text">Are you sure you want to reject this application?</p>
                                                    <!-- You can add more details or context here if needed -->
                                                        ...
                                                        <option value="Invalid / Incorrect Information">Invalid / Incorrect Information</option>
                                                            ...
                                                            <option value="Insufficient Fund of CRG-MPC">Insufficient Funds of CRG-MPC</option>
                                                ...
                                                <div class="modal-footer">
                                                    ...
                                                    <button class="btn btn-danger btn-icon-split m-1" type="submit" name="disapprove-btn"><span class="text-white-50 icon"><i class="fas fa-times-circle"></i></span><span class="text-white text">Reject</span></button>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="modal fade" role="dialog" tabindex="-1" id="approve-modal">
                                        <div class="modal-dialog modal-dialog-centered" role="document">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                    <p class="p-text">Are you sure you want to forward this application to receive approval from the Chairperson? By doing so, you acknowledge that this application has passed strict evaluation and is qualified to receive approval.</p>
                                                    <!-- You can add more details or context here if needed -->
                                                </div>
                                                <div class="modal-footer">
                                                    <button class="btn btn-secondary btn-icon-split m-1" type="button" data-bs-dismiss="modal"><span class="text-white-50 icon"><i class="fas fa-chevron-circle-left"></i></span><span class="text-white text">Cancel</span></button>
                                                    <!-- You can add an action to disapprove here -->
                                                    <button class="btn btn-primary btn-icon-split m-1" type="submit" name="approve-btn"><span class="text-white-50 icon"><i class="fas fa-chevron-circle-right"></i></span><span class="text-white text">Forward</span></button>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="row row-cols-1 row-cols-sm-1">
                                        ...
                                        <div class="col d-flex flex-column justify-content-end col-md-3 col-style"><label class="form-label">Seq Nos.</label><input class="form-control d-lg-flex" type="text" id="user-id" autocomplete="on" name="seq-nos"></div>
                                        <div class="col d-flex flex-column justify-content-end col-md-3 col-style"><label class="form-label">PB Lot/Nos.</label><input class="form-control d-lg-flex" type="text" id="user-id" autocomplete="on" name="lot-nos" ></div>
                                    </div>
                                    ...
                                    <div class="row row-cols-1 row-cols-sm-1">
                                        ...
                                        <div class="col d-flex flex-column justify-content-end col-md-4 col-style"><label class="form-label">Renewal Balance<span style="color: rgb(231, 74, 59);">*</span></label>
                                            <input class="form-control d-lg-flex" type="number" id="renewal-balance-amount" name="renewal-balance-amount" >
                                    </div>
                            ...
                            <div class="card-footer">
                                ...
                                <a class="btn btn-danger btn-icon-split m-1" data-bs-target="#disapprove-modal" data-bs-toggle="modal"><span class="text-white-50 icon"><i class="fas fa-times-circle"></i></span><span class="text-white text">Reject</span></a>
                                <a class="btn btn-primary btn-icon-split m-1" data-bs-target="#approve-modal" data-bs-toggle="modal"><span class="text-white-50 icon"><i class="fas fa-chevron-circle-right"></i></span><span class="text-white text">Forward</span></a>
                            </div>
                        </div>
                    </form>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    // Get the selected value of loan-status
    var selectedValue = document.getElementById('loan-status').value;
    
    // Get the balance-amount input element
    var balanceAmountInput = document.getElementById('renewal-balance-amount');
    
    // Disable the balance-amount input based on the selected value
    if (selectedValue === 'Renew') {
        balanceAmountInput.disabled = false;
    } else if (selectedValue === 'New'){
        balanceAmountInput.disabled = true;
    }
});
