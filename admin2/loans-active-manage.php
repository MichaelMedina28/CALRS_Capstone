...
if (isset($_POST['approve'])) {
    ...
    $description = "$adminUsername completed loan ($loan_id).";
    ...
    //For Activity Logs
    $description = "$adminUsername completed your loan ($loan_id). You can now check your loan in Completed Loans";
    $logSql1 = "INSERT INTO notif_logs (user_id, notif_description) VALUES ('$user_id', '$description')";
...
if (isset($_POST['payment'])) {
    ...
    //For Login Logs
    $adminUsername = $_SESSION["admin_username"];
    $description = "$adminUsername updated the payment for loan ($loan_id).";
    $logSql = "INSERT INTO login_logs (user_id, notif_description) VALUES ('$adminUsername', '$description')";
    $conn->query($logSql); // Assuming $conn is your database connection

    //For Activity Logs
    $description = "$adminUsername updated your payment.";
    $logSql1 = "INSERT INTO notif_logs (user_id, notif_description) VALUES ('$user_id', '$description')";
    $conn->query($logSql1); // Assuming $conn is your database connection

    header("Location: loans-active-manage.php?id=$loan_id");
}
...
<body id="page-top">
                    ...
                    <form method="post">
                        <div class="card shadow">
                            ...
                            <div class="card-footer">
                                ...
                                <button class="btn btn-primary btn-icon-split m-1" name="update" id="payment-btn" type="button" data-bs-target="#login-register-modal" data-bs-toggle="modal"><span class="text-white-50 icon"><i class="fas fa-edit text-light"></i></span><span class="text-white text">Make a Payment</span></button>


                                <!-- Payment Modal -->
                                <div class="modal fade" role="dialog" tabindex="-1" id="login-register-modal">
                                    <div class="modal-dialog modal-dialog-centered" role="document">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title"><strong>Make a Payment</strong></h5><button class="btn-close btn-close-white" type="button" aria-label="Close" data-bs-dismiss="modal"></button>
                                            </div>
                                            <div class="modal-body">
                                                <div class="row">
                                                    <div class="col">
                                                        <label class="form-label">Reference No.</label>
                                                        <input class="form-control" id="ref-no" type="number" required="" name="ref-no" value="<?php echo $lastRefNo; ?>" readonly>
                                                    </div>
                                                </div>
                                                <div class="row">
                                                    <div class="col">
                                                        <label class="form-label">Payment Date</label>
                                                        <input class="form-control" id="pl-date" type="text" required="" name="pl-date" value="<?php echo $nextAmortizationDate; ?>">
                                                    </div>
                                                </div>
                                                <div class="row">
                                                    <div class="col"><label class="form-label">Payment Amount</label><input class="form-control" id="pl-payment" type="text" name="pl-payment" value="<?php echo $da_amortization_amount1; ?>" readonly></div>
                                                </div>
                                                <div class="row mt-2">
                                                    <div class="col"><label class="form-label text-success"><strong>Remaining Balance After Payment</strong></label><input class="form-control" id="pl-balance" type="text" required="" name="pl-balance" value="<?php echo $row["al_amount_balance"] ?>" readonly></div>
                                                </div>
                                                <div class="row">
                                                    <div class="col"><label class="form-label">Remarks</label>
                                                        <select class="form-control" id="pl-remarks" name="pl-remarks" onchange="paymentRemarks()">

                                                            <option id="due" value="Paid Due Amount (<?php echo date('m/d/Y'); ?>)" <?php if (strtotime($nextAmortizationDate) !== strtotime(date('m/d/Y'))) { echo 'style="display: none;"'; } ?>>Paid Due Amount (<?php echo date('m/d/Y'); ?>)</option>

                                                            <option id="adv" value="Paid Advanced Payment (<?php echo date('m/d/Y'); ?>)" <?php if (strtotime($nextAmortizationDate) < strtotime(date('m/d/Y'))) { echo 'style="display: none;"'; } ?>>Paid Advanced Payment (<?php echo date('m/d/Y'); ?>)</option>

                                                            <option id="ovr" value="Paid Overdue Payment (<?php echo date('m/d/Y'); ?>)" <?php if (strtotime($nextAmortizationDate) > strtotime(date('m/d/Y'))) { echo 'style="display: none;"'; } ?>>Paid Overdue Payment (<?php echo date('m/d/Y'); ?>)</option>
                                                        </select>
                                                    </div>
                                                </div>
                                            ...
                                            <div class="modal-footer">
                                                ...
                                                <button class="btn btn-primary btn-icon-split m-1" name="payment" id="payment" type="submit"><span class="text-white-50 icon"><i class="fas fa-check-circle text-light"></i></span><span class="text-white text">Confirm Payment</span></button>
                                            </div>
                                        </div>
                                    </div>
                                </div>

                                <button class="btn btn-success btn-icon-split m-1" name="approve" id="complete-payment-btn" type="submit" disabled><span class="text-white-50 icon"><i class="fas fa-check-circle"></i></span><span class="text-white text">Complete Loan</span></button>
                            </div>
                        </div>
                    </form>
    ...
    <script>
        var formattedNewBalance;
        var balanceDefaultValue;
        ...
        function calculateBalance() {
            ...
            if (formattedNewBalance < 1) {
                formattedNewBalance = "0";
            }

            // Update the balance input field with the formatted result
            balanceInput.value = formattedNewBalance;
            balanceDefaultValue = formattedNewBalance;

        }


        var installment = document.getElementById("installment").value;
        var deductionAmount = parseFloat(<?php echo $da_deduction_amount; ?>);
        if (installment === "Monthly") {
            deductionAmount /= 12;
        } else if (installment === "Quarterly") {
            deductionAmount /= 3;
        } else if (installment === "Semi-Annually") {
            deductionAmount /= 2;
        }

        // Get a reference to the option element


        function paymentRemarks() {
            // Get the current values from the input fields
            const dateDefaultValue = document.getElementById("pl-date");
            const amountDefaultValue = document.getElementById("amount-per-payment");

            var dateDefault = dateDefaultValue.value;
            var amountDefault = amountDefaultValue.value;
            var balanceDefault = balanceDefaultValue;

            var currentDate = new Date();
            var PaymentDate = new Date(dateDefault);

            const monthsDiff = (currentDate.getFullYear() - PaymentDate.getFullYear()) * 12 + (currentDate.getMonth() - PaymentDate.getMonth());

            var newDeductionAmount = deductionAmount * monthsDiff;
            console.log(newDeductionAmount);

            var amountParseDefault = parseFloat(amountDefault.replace(/,/g, ''));
            var balanceParseDefault = parseFloat(balanceDefault.replace(/,/g, ''));
            ...
            var dueOption = document.getElementById("due");
            var advOption = document.getElementById("adv");
            var ovrOption = document.getElementById("ovr");

            if (advOption.selected === true) {
                // Store the former values
                var advOption = document.getElementById("adv");
                // Set the content of the option with the variable value
                advOption.textContent = "Paid Advanced Payment (-₱" + deductionAmount + ") (<?php echo date('m/d/Y'); ?>)";
                // Optional: Set the value attribute as well
                advOption.value = "Paid Advanced Payment (-₱" + deductionAmount + ") (<?php echo date('m/d/Y'); ?>)";

                var formerBalance = balanceParseDefault;
                var formerAmount = amountParseDefault;

                // Subtract the deduction amount
                newBalance = formerBalance;
                newAmount = formerAmount - deductionAmount * monthsDiff;

                if (newBalance < 1) {
                    newBalance = "0";
                }

                var formattedBalance = newBalance.toLocaleString();
                var formattedAmount = newAmount.toLocaleString();


                balanceInput.value = formattedBalance;
                amountInput.value = formattedAmount;

            } else if (ovrOption.selected === true) {

                var ovrOption = document.getElementById("ovr");
                // Set the content of the option with the variable value
                ovrOption.textContent = "Paid Overdue Payment (+₱" + newDeductionAmount + ") (<?php echo date('m/d/Y'); ?>)";
                // Optional: Set the value attribute as well
                ovrOption.value = "Paid Overdue Payment (+₱" + newDeductionAmount + ") (<?php echo date('m/d/Y'); ?>)";
            ...
            } else if (dueOption.selected === true) {
                // Revert to default values
                newBalance = balanceParseDefault;
                newAmount = amountParseDefault;
        ...
        var balance = document.getElementById("da-amount-balance").value;
