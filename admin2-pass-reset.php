...
if (isset($_GET['token'])) {
    ...
    $stmt = $conn->prepare('SELECT * FROM admin2_tbl WHERE reset_token = ?');
    ...
    if ($result->num_rows > 0) {
        ...
        if ($row['password_reset_done'] == 0) {
            try {
                ...
                $updateStmt = $conn->prepare('UPDATE admin2_tbl SET admin_pass = ?, admin_pass_confirm = ?, password_reset_done = 1 WHERE reset_token = ?');
...
<body>
    ...
    <div class="container"><a class="navbar-brand d-flex align-items-center" href="index-admin2.php"><img id="login-img" src="assets/img/logo/logo.png"><span><strong>CRG-MPC</strong></span></a></div>
