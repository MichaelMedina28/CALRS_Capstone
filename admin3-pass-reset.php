...
if (isset($_GET['token'])) {
    ...
    $stmt = $conn->prepare('SELECT * FROM admin3_tbl WHERE reset_token = ?');
    ...
    if ($result->num_rows > 0) {
        ...
        if ($row['password_reset_done'] == 0) {
            try {
                ...
                $updateStmt = $conn->prepare('UPDATE admin3_tbl SET admin_pass = ?, admin_pass_confirm = ?, password_reset_done = 1 WHERE reset_token = ?');
